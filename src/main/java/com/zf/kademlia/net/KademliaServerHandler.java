        ...
        if (message.getType() == MessageType.PING) {
            ...
            if (localStorage.contains(findValue.getKey())) {
                ctx.writeAndFlush(new DatagramPacket(codec.encode(new ValueReply(message.getSeqId(), localNode,
                        findValue.getKey(), localStorage.get(findValue.getKey()).getContent())), packet.sender()));
            } else {
            ...
            localStorage.put(store.getKey(), Value.builder().content(store.getValue()).lastPublished(System
